...
{
    internal class CostSearcher: IDisposable
    {
        public int currCost { get { return mCurrentCost; } }
        int mCurrentCost = 0;
        ...
        public bool SearchCost()
        {
            mDriver.Navigate().GoToUrl(mStrUrl);
            string strCost = "";
            var currCostField = mDriver.FindElementById("nowVal_td_0");

            if (currCostField.Displayed)
            {
                strCost = currCostField.Text;
            }

            strCost = strCost.Replace(",", "");
            mCurrentCost = int.Parse(strCost);
            mDriver.Navigate().GoToUrl("https://finance.naver.com/");
            return true;
        }
